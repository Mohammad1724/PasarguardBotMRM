...
def _atomic_write_text(path: Path, content: str, *, encoding: str = "utf-8") -> None:
    ...
    Falls back to direct write if the path is on a bind mount (e.g. Docker).
    """
    # Try atomic write first (write to temp, then os.replace)
    fd = None
    tmp_path = None
    try:
        ...
        # Bind mount or read-only — fall back to direct write
        pass
    finally:
        if fd is not None:
            os.close(fd)
    ...
    # Fallback: direct write (works on bind mounts if not :ro)
