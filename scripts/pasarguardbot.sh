...
action_restore() {
    ...
    is_installed || die "Install the bot first."
    command -v flock >/dev/null || die "flock (util-linux) is required."
    # Local lock covers service stop through restore and restart. The Python layer
    # also holds a renewable ownership-checked Redis lock. Never import while live.
    exec 9>"${CONFIG_DIR}/restore.lock"
    flock -n 9 || die "Another restore is in progress."
    echo "This replaces all data. Only restore a trusted backup."
    echo "A private safety backup is saved before deleting tables."
    local confirm
    read -r -p "Type RESTORE to continue: " confirm || true
    [[ "$confirm" == "RESTORE" ]] || { info "Cancelled"; flock -u 9; exec 9>&-; return 0; }
    local status=0
    if is_native_mode; then
        systemctl stop pasarguardbot.service || die "Cannot stop bot; refusing restore."
        if systemctl is-active --quiet pasarguardbot.service; then
            die "Bot still running; refusing restore."
        fi
        systemctl start pasarguardbot-mariadb.service pasarguardbot-redis.service || die "Cannot start database/Redis."
        (
            cd "$APP_DIR" || exit 1
            uv run --no-sync python -m app.services.restore_cli "$zip_path" --offline \
    ...
    local do_restart
    read -r -p "Restart bot now? (Y/n): " do_restart || true
...
action_miniapp() {
    ...
    tmp="$(mktemp)"
    if ! curl_download "https://raw.githubusercontent.com/Mohammad1724/PasarguardBotMRM/${branch}/scripts/setup-miniapp.sh" "$tmp"; then
        rm -f "$tmp"
